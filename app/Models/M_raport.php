    ...
    public function getPenilaianWithJoins($where)
{
    $builder = $this->db->table('penilaian')
        ->select('penilaian.*, siswa.nama_siswa, siswa.id_rombel, blok.nama_blok, mapel.nama_mapel, semester.semester, tahun_ajaran.tahun_ajaran')
        ->join('siswa', 'penilaian.id_siswa = siswa.id_siswa')
        ->join('blok', 'penilaian.id_blok = blok.id_blok')
        ->join('mapel', 'penilaian.id_mapel = mapel.id_mapel')
        ->join('semester', 'penilaian.id_semester = semester.id_semester')
        ->join('tahun_ajaran', 'penilaian.id_tahun_ajaran = tahun_ajaran.id_tahun_ajaran')
    ...
    public function join2tbl($tabel1, $tabel2, $on1)
    {
        return $this->db->table($tabel1)
            ->join($tabel2, $on1, 'inner')
            ->get()
    ...
    public function join1tbl($tabel1, $tabel2, $on1)
{
    return $this->db->table($tabel1)
        ->join($tabel2, $on1, 'inner')
        ->get()
...
{
    return $this->db->table('rombel')->select('id_rombel, nama_rombel')->get()->getResult();
}
    ...
    public function getJadwal($id_rombel, $id_blok, $id_tahun_ajaran)
    {
        return $this->db->table('jadwal')
            ->select('jadwal.sesi, mapel.nama_mapel, guru.nama_guru, jadwal.id_jadwal, jadwal.id_guru, jadwal.id_mapel')
            ...
            ->where('jadwal.id_tahun_ajaran', $id_tahun_ajaran)
            ->get()
            ->getResult();
    }
    ...
    public function tampil_siswa()
    {
        return $this->db->table('siswa')
            ...
            ->select('siswa.id_siswa, siswa.nama_siswa, rombel.nama_rombel')
            ->get()
            ->getResult();
    }
    ...
    public function updateJadwal($id_jadwal, $sesi, $nama_mapel, $nama_guru) {
        return $this->db->table('jadwal')
            ->where('id_jadwal', $id_jadwal)
            ->update([
                'sesi' => $sesi,
    ...
    public function getSiswaData($id_siswa)
    {
        return $this->db->table('siswa')
            ->select('siswa.nama_siswa, siswa.nis, rombel.nama_rombel, semester.nama_semester')
            ->join('rombel', 'siswa.id_rombel = rombel.id_rombel')
            ->join('semester', 'siswa.id_semester = semester.id_semester')
            ->where('siswa.id_siswa', $id_siswa)
            ->get()
            ->getRow(); // Fetch single result
    }
    
    public function getPenilaianData($id_siswa, $id_tahun_ajaran, $id_semester, $id_blok)
{
    return $this->db->table('penilaian')
        ->select('penilaian.*, siswa.nama_siswa, siswa.nis, rombel.nama_rombel, semester.semester, mapel.nama_mapel')
        ->join('siswa', 'siswa.id_siswa = penilaian.id_siswa')
        ->join('rombel', 'rombel.id_rombel = siswa.id_rombel')
        ->join('semester', 'semester.id_semester = penilaian.id_semester')
        ->join('mapel', 'mapel.id_mapel = penilaian.id_mapel') // Join ke tabel mapel
        ->where('penilaian.id_siswa', $id_siswa)
        ->where('penilaian.id_tahun_ajaran', $id_tahun_ajaran)
        ->where('penilaian.id_semester', $id_semester)
        ->where('penilaian.id_blok', $id_blok)
        ->get()
        ->getResult();
}
